...
?>
			<tr><td class="fn"><a href="<?php echo $ui->url(array('fn' => Utils::safe_fn($info['fn'])));?>"><?php echo $info['fn'];?></a></td>
<?php if ($ui->display_calls) { 
		echo Utils::td_num($info['ct'], $ui->config->format_cbk['ct'], ($ui->sort == 'ct'));
		//echo Utils::td_pct($info['ct'], $ui->totals[0]['ct'], ($ui->sort == 'ct'));
}?>
...
<?php	foreach ($ui->metrics as $metric) { 
		...
		//echo Utils::td_pct($info[$metric], $ui->totals[0][$metric], ($ui->sort == $metric));

		// Exclusive Metric
		echo Utils::td_num($info['excl_'.$metric], $ui->config->format_cbk['excl_' . $metric], ($ui->sort == 'excl_' . $metric));
		//echo Utils::td_pct($info['excl_'.$metric], $ui->totals[0][$metric], ($ui->sort == 'excl_' . $metric));
