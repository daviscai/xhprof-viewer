...
?>
		<h2>Parent/Child report for <strong><?php echo $ui->fn;?></strong> <br/> (Run <a href="<?php echo $ui->url(array('fn' => null));?>">#<?php echo $ui->runs[0]->run_id;?></a>)</h3>
...
if ($ui->display_calls) {
	echo Utils::td_num($data[$ui->fn]['ct'], $ui->config->format_cbk['ct'], ($ui->sort == 'ct'));
	//echo Utils::td_pct($data[$ui->fn]['ct'], $ui->totals['ct'], ($ui->sort == 'ct'));
}

foreach ($ui->metrics as $metric) {
	// Inclusive metric
	echo Utils::td_num($data[$ui->fn][$metric], $ui->config->format_cbk[$metric], ($ui->sort == $metric));
	//echo Utils::td_pct($data[$ui->fn][$metric], $ui->totals[$metric], ($ui->sort == $metric));
}
...
foreach ($ui->metrics as $metric) {
	// Inclusive metric
	echo Utils::td_num($data[$ui->fn]['excl_'.$metric], $ui->config->format_cbk['excl_'.$metric], ($ui->sort == $metric), 'Child', $metric);
	//echo Utils::td_pct($data[$ui->fn]['excl_'.$metric], $ui->totals[$metric], ($ui->sort == $metric), 'Child', $metric);
}
...
?>
			...
			if ($ui->display_calls) {
				echo Utils::td_num($info['ct'], $ui->config->format_cbk['ct'], ($ui->sort == 'ct'));
				//echo Utils::td_pct($info['ct'], $base_ct[$pc], ($ui->sort == 'ct'));
			}
			foreach ($ui->metrics as $metric) {
				// Inclusive metric
				echo Utils::td_num($info[$metric], $ui->config->format_cbk[$metric], ($ui->sort == $metric));
				//echo Utils::td_pct($info[$metric], $pc == 'parent' ? $ui->totals[$metric] : $base_info[$metric], ($ui->sort == $metric));
			}
	?></tr>
<?php 
		}
