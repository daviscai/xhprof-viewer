...
?>
		...
		if ($ui->display_calls) {
			echo Utils::td_num($ui->totals[1]['ct'] - $ui->totals[0]['ct'], $ui->config->format_cbk['ct'], true, true);
			echo Utils::td_pct($ui->totals[1]['ct'] - $ui->totals[0]['ct'], $ui->totals[0]['ct'], true, true);
		}

		foreach ($ui->metrics as $metric) {
			// Inclusive metric
			echo Utils::td_num($ui->totals[1][$metric] - $ui->totals[0][$metric], $ui->config->format_cbk[$metric], true, true);
			echo Utils::td_pct($ui->totals[1][$metric] - $ui->totals[0][$metric], $ui->totals[0][$metric], true, true);
...
?>
			<tr><td class="fn"><a href="<?php echo $ui->url(array('fn' => Utils::safe_fn($info['fn'])));?>"><?php echo $info['fn'];?></a></td><?php
	if ($ui->display_calls) {
		echo Utils::td_num($info['ct'], $ui->config->format_cbk['ct'], ($ui->sort == 'ct'), true);
		echo Utils::td_pct($info['ct'], $ui->totals[2]['ct'], ($ui->sort == 'ct'), true);
	}

	foreach ($ui->metrics as $metric) {
		// Inclusive metric
		echo Utils::td_num($info[$metric], $ui->config->format_cbk[$metric], ($ui->sort == $metric), true);
		echo Utils::td_pct($info[$metric], $ui->totals[2][$metric], ($ui->sort == $metric), true);

		// Exclusive Metric
		echo Utils::td_num($info['excl_'.$metric], $ui->config->format_cbk['excl_' . $metric], ($ui->sort == 'excl_' . $metric), true);
		echo Utils::td_pct($info['excl_'.$metric], $ui->totals[2][$metric], ($ui->sort == 'excl_' . $metric), true);
	}
?></tr>
<?php 
}
