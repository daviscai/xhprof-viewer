...
use app\modules\xhprof\report\Single;
use app\modules\xhprof\report\Diff;
...
class XHProfUI {
	...
	*/
	...
	function __construct($params, Config $config, $dir = null) {
		$this->params = Utils::parse_params($params);
	...
	function generate_report() {
		...
		if ($run) {
			...
			foreach (explode(',', $run) as $run_id) {
				$this->runs[] = new Run($this->dir, $namespace, $run_id);
			}

			if ($compare) {
				$this->runs = array(
					$this->runs[0],
					new Run($this->dir, $namespace, $compare)
				);

				if (($data1 = $this->runs[0]->get_data()) && ($data2 = $this->runs[1]->get_data())) {
					$this->_setup_metrics($data2);

					return new Diff($this, $data1, $data2);
				}
