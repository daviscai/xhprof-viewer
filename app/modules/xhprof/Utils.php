...
class Utils {
	...
	public static function list_runs($dir) {
		echo '<div class="container"><hr/>'.Wen::t('Existing runs').': <br/><ul>';
	 ...
	 */
	public static function sort_cbk($a, $b, XHProfUI $ui) {
		if ($ui->sort == 'fn') {
	 ...
	 */
	public static function td_class($num, $bold, $diff_mode = false) {
		if ($bold) {
			if ($diff_mode) {
				if ($num <= 0) {
	...
	public static function td_num($num, $fmt_func, $bold = false, $diff_mode = false) {
		...
		if (!empty($fmt_func)) {
			if(is_array($fmt_func)){
				$num = Utils::$fmt_func[0]($num);
			}else{
				$num = call_user_func($fmt_func, $num);
			}
		}

		return "<td class=\"center $class\"  colspan=\"2\">$num</td>";
	}
	...
	public static function td_pct($numer, $denom, $bold = false, $diff_mode = false) {
		...
		return "<td class=\"center $class\"  colspan=\"2\">$pct</td>";
	}
